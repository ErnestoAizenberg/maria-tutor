    ...
    def __call__(self, request):
        ...
        try:
            ...
            if 400 <= response.status_code < 600:
                self.logger.warning(f"Error response detected: {response.status_code} for {request.path}")
                return self.render_error_page(request, response.status_code)

            return response
        except Exception as e:
            self.logger.error(f"Unhandled exception in middleware: {str(e)}", exc_info=True)
            raise
