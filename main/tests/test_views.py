...
class ViewTests(TestCase):
    ...
    def test_articles_view(self):
        ...
        self.assertEqual(len(response.context['articles']), 1)  # Only published articles
    ...
    def test_connect_request_valid(self):
        response = self.client.post(reverse('connect_request'), {
            'name': 'Test User',
            'email': 'test@example.com',
    ...
    def test_subscribe_email_valid(self):
        response = self.client.post(reverse('subscribe_email'), {
            'email': 'test@example.com'
        })
